...
def anisotropy_angle( case_seed ):
...
def generate_random_fields(case_num , case_seed):
    ...
    cond_srf = gs.CondSRF(krige, seed=case_num*case_seed)
    # field = cond_srf.set_pos([x, y], "structured")
    #
    # srf = gs.SRF(model, mean=np.log(80.0), seed=case_num* case_seed)
...
def generate_random_fields555(case_num,case_seed):

    mesh = toughio.read_mesh("MESH")


    model = gs.Gaussian(dim=2, var=0.2, len_scale=10)
    srf = gs.SRF(model, seed=case_num * case_seed)
    ...
    plt.savefig(f"{case_num}Permeability.png")
    ...
    plt.savefig(f"{case_num}Porosity.png")
...
def ini_incon(seed_num , case_seed):
    ...
    # np.random.seed(seed_num * case_seed)
...
def ini_incon555(seed_num, case_seed):
...
def run_toughreact555(case_num):
    ...
    folder_name = 'case_' + str(case_num)
    command = rf"D:\\toughreact-run\\{folder_name}\\treact_eco2n "
...
def make_dynamic_injection(case_num , case_seed):
    f = toughio.read_input('input0.txt')
    time_seq = list(np.arange(0., 155520000. + 2592000., 2592000.))
    tmp = latin_sampling(2.1, 15.1, len(time_seq), case_num * case_seed)
    rates_seq = []
    for rate in tmp:
...
def make_dynamic_injection555(case_num, num_wells, group_size, num_cases_per_group , case_seed):
    f = toughio.read_input('GENER', file_format="toughreact-flow")

    time_seq = list(np.arange(0., 86400 * 365 + 86400 * 5, 86400 * 5))

    total_rate = latin_sampling(3., 5., 1, case_num * case_seed)
...
def make_constance_injection3(case_num, num_wells, group_size, num_cases_per_group , case_seed):
    f = toughio.read_input('GENER', file_format="toughreact-flow")
...
def make_constance_injection5(case_num, num_wells, group_size, num_cases_per_group , case_seed):
    f = toughio.read_input('GENER', file_format="toughreact-flow")
...
def gene_routin(case_num, pro_path, case_seed):
    print(f'---------- case {case_num} begins ----------')
    case_begin_time = time.time()
    new_folders(case_num)
    change_dir(case_num)
    # anisotropy_angle()
    generate_random_fields555(case_num,case_seed)
    ini_incon555(case_num , case_seed)
    make_dynamic_injection555(case_num , 30 , 10 , 55 , case_seed)
...
def gene_routin3(case_num, pro_path, case_seed):
    print(f'---------- case {case_num} begins ----------')
    case_begin_time = time.time()
    new_folders(f'{case_num}_3')
    change_dir(case_num)
    # anisotropy_angle()
    generate_random_fields555(case_num,case_seed)
    ini_incon555(case_num , case_seed)
    make_constance_injection3(case_num , 30 , 10 , 55 , case_seed)
    run_toughreact555(case_num)
    os.chdir(pro_path)
    case_end_time = time.time()
    print(f'case {case_num} finished. {(case_end_time - case_begin_time) / 60.:.2f} mins used.' + '\n')
...
def gene_routin5(case_num, pro_path, case_seed):
    print(f'---------- case {case_num} begins ----------')
    case_begin_time = time.time()
    new_folders(f'{case_num}_5')
    change_dir(case_num)
    # anisotropy_angle()
    generate_random_fields555(case_num,case_seed)
    ini_incon555(case_num , case_seed)
    make_constance_injection5(case_num , 30 , 10 , 55 , case_seed)
    run_toughreact555(case_num)
    os.chdir(pro_path)
    case_end_time = time.time()
    print(f'case {case_num} finished. {(case_end_time - case_begin_time) / 60.:.2f} mins used.' + '\n')
...
if __name__ == '__main__':
    gene_routin(104,os.getcwd(),case_seed=55)
